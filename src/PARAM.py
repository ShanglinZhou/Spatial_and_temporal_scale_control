...
# Runtime mode can be selected without editing the archived source copy:
# $env:RNN_MODE='train'; python src\PARAM.py
Mode = os.environ.get('RNN_MODE', 'eval').strip().lower() # eval, train
if Mode not in ('eval', 'train'):
    raise ValueError("RNN_MODE must be 'eval' or 'train', got {!r}".format(Mode))

# Current training task. The environment override avoids editing this file:
# $env:RNN_TASK='motorTraj_circle_time'; python src\PARAM.py
SUPPORTED_TASKS = (
    'motorTraj_circle',
...
# $env:RNN_DEVICE='cuda'; python src\PARAM.py
...
LOWRANK_EVAL_DYNAMICS_DIR = os.environ.get(
    ...
    os.path.join(os.path.dirname(os.path.abspath(__file__)), 'results', 'motorTraj_circle'),
)
